...
public class Piece extends StackPane {
	...
	public Piece(PieceType type, int x, int y) {
		...
		piece.setFill(type == PieceType.RED ? Color.valueOf("#ff0000") : Color
				.valueOf("#ffffff"));
		...
		setOnMouseDragged(e -> {
			relocate(e.getSceneX() - mouseX + oldX, e.getSceneY() - mouseY
					+ oldY);
		});
	}
	...
	public void coronation() {
		if (type == PieceType.REDKING) {
			image = new javafx.scene.image.Image(
					ClassLoader.getSystemResourceAsStream("crownRed.png"));
		} else {
			image = new javafx.scene.image.Image(
					ClassLoader.getSystemResourceAsStream("crownWhite.png"));
		}
